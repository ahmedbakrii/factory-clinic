...
export default function EmployeesPage() {
  ...
  // إغلاق قائمة الإكسيل عند الضغط خارجها
  useEffect(() => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const [deptsRes, natsRes] = await Promise.all([
  ...
  const openEmployeeHistory = async (emp: any) => {
    ...
    try {
      const { data, error } = await supabase
        .from("visits")
        .select(`*, visit_vitals(*), work_injuries(*)`)
        .eq("employee_id", emp.id)
        .order("created_at", { ascending: false });
      if (error) throw error;
      setEmpHistory(data || []);
    } catch (error) { console.error("Error:", error); } finally { setIsHistoryLoading(false); }
  };

  const handleAddEmployee = async (e: React.FormEvent) => {
    e.preventDefault();
    const session = JSON.parse(localStorage.getItem("clinic_session") || "{}");
    if (session.id === "DEMO") return alert("👁️ وضع المشاهدة (Demo Mode) مفعل. غير مصرح لك بالإضافة.");
  ...
  const handleUpdateEmployee = async (e: React.FormEvent) => {
    ...
    const session = JSON.parse(localStorage.getItem("clinic_session") || "{}");
    if (session.id === "DEMO") {
      alert("👁️ وضع المشاهدة (Demo Mode) مفعل. غير مصرح لك بتعديل بيانات الموظفين.");
      setEditingEmp(null);
      return;
    }
  ...
  const downloadTemplate = async () => {
    ...
    setIsExcelMenuOpen(false); // إغلاق القائمة بعد التحميل
  };
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsExcelMenuOpen(false); // إغلاق القائمة
    setIsLoading(true);
    try {
      const XLSX = await import("xlsx");
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="relative w-full sm:w-auto" ref={excelMenuRef}>
            ...
            <button 
              ...
              className="w-full sm:w-auto bg-white border border-emerald-200 text-emerald-700 hover:bg-emerald-50 px-4 py-2.5 rounded-xl font-bold flex items-center justify-between sm:justify-center gap-2 transition-colors shadow-sm"
            >
            ...
            {isExcelMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-full sm:w-56 bg-white border border-slate-100 shadow-2xl rounded-2xl overflow-hidden z-30 animate-in fade-in zoom-in-95">
                <button onClick={downloadTemplate} className="w-full text-right px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition-colors border-b border-slate-50 flex items-center gap-2">
                  <Download size={16} /> تحميل قالب البيانات
                </button>
                <button onClick={() => { fileInputRef.current?.click(); setIsExcelMenuOpen(false); }} className="w-full text-right px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-emerald-600 transition-colors flex items-center gap-2">
                  <FileUp size={16} /> رفع ملف الموظفين
                </button>
              </div>
            )}
          </div>

          <button onClick={() => setShowAddModal(true)} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors shadow-sm shadow-emerald-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredEmployees.map((emp) => (
            <div 
              ...
              className="bg-white rounded-[24px] shadow-sm border border-slate-100 p-5 hover:shadow-md transition-all flex flex-col h-full relative overflow-hidden group cursor-pointer"
            >
              
              {emp.is_chronic && (
                <div className="absolute top-4 left-4 bg-red-100 text-red-700 px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1">
                  <AlertCircle size={12} /> مرض مزمن
                </div>
              )}

              <button 
                onClick={(e) => { e.stopPropagation(); setEditingEmp(emp); }} 
                className="absolute top-4 left-4 md:left-auto md:right-4 bg-slate-100 hover:bg-blue-100 text-slate-500 hover:text-blue-600 p-2 rounded-xl transition-colors md:opacity-0 group-hover:opacity-100 z-10"
                style={{ display: emp.is_chronic ? 'none' : 'block' }}
              >
                <Edit size={16} />
              </button>
              ...
              <div className="flex items-start gap-4 mb-4 mt-2">
                ...
                <div className="flex-1 min-w-0 pt-1">
                  <div className="flex items-center gap-2">
                    ...
                    {emp.is_chronic && (
                      <button onClick={(e) => { e.stopPropagation(); setEditingEmp(emp); }} className="text-slate-400 hover:text-blue-600 z-10 relative">
                        <Edit size={14}/>
                      </button>
                    )}
                  </div>
              ...
              <Link 
                href={`/visit/new?iqama=${emp.iqama_number}`} 
                onClick={(e) => e.stopPropagation()} 
                className="w-full bg-white border border-blue-200 text-blue-600 hover:bg-blue-600 hover:text-white hover:border-blue-600 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors relative z-10"
              >
                <Activity size={16} /> تسجيل زيارة للموظف
              </Link>
          ...
          <div className="bg-[#f4f7f6] rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-white sticky top-0 z-10">
              <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2">
                <Edit size={20} className="text-blue-600"/> تعديل بيانات الموظف
              </h2>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                <div className="border-b border-gray-100 px-6 py-4 flex items-center justify-between">
                  <h2 className="font-bold text-gray-800 text-lg flex items-center gap-2">بيانات العامل (Employee Info)</h2>
                  <User size={22} className="text-blue-500" />
                </div>
                <div className="p-6 space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 md:col-span-1">
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">الاسم (Name)</label>
                      <input type="text" value={editingEmp.name} onChange={e => setEditingEmp({...editingEmp, name: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-blue-500 text-center bg-gray-50/50" />
                    </div>
                    <div className="col-span-2 md:col-span-1">
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">رقم الجوال (Phone)</label>
                      <input type="text" value={editingEmp.phone || ''} onChange={e => setEditingEmp({...editingEmp, phone: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-blue-500 text-center bg-gray-50/50" dir="ltr" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">الجنسية (Nationality)</label>
                      <AutocompleteInput options={dbNationalities} value={editingEmp.nationality || ''} onChange={(val) => setEditingEmp({...editingEmp, nationality: val})} placeholder="اختر أو اكتب الجنسية..." />
                    </div>
                    <div>
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">رقم الإقامة</label>
                      <input type="text" value={editingEmp.iqama_number} disabled className="w-full p-3 border border-gray-200 rounded-xl bg-gray-100 text-center text-gray-500 cursor-not-allowed" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">القسم/المكان (Work place)</label>
                      <AutocompleteInput options={dbDepartments} value={editingEmp.department || ''} onChange={(val) => setEditingEmp({...editingEmp, department: val})} placeholder="اختر أو اكتب القسم..." />
                    </div>
                    <div>
                      <label className="block text-center text-sm font-semibold text-gray-600 mb-2">المشرف (Supervisor)</label>
                      <input type="text" value={editingEmp.work_place || ''} onChange={e => setEditingEmp({...editingEmp, work_place: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-blue-500 text-center bg-gray-50/50" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-red-50 border border-red-100 rounded-2xl p-6">
                ...
                {editingEmp.is_chronic && (
                  <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                    <label className="block text-sm font-bold text-red-800 mb-2">ملاحظات طبية (Medical Notes)</label>
                    <textarea 
                      placeholder="اكتب تفاصيل المرض (ضغط، سكر، قلب، ربو...)" 
                      value={editingEmp.chronic_disease_notes || ''} 
                      onChange={e => setEditingEmp({...editingEmp, chronic_disease_notes: e.target.value})} 
                      className="w-full p-4 border border-red-200 rounded-xl outline-none focus:border-red-400 resize-none bg-white"
                      rows={2}
                    ></textarea>
                  </div>
                )}
          ...
          <div className="bg-[#f8fafc] rounded-[24px] w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom-8">
            ...
            {selectedEmp.is_chronic && selectedEmp.chronic_disease_notes && (
              <div className="bg-red-50 border-b border-red-100 p-4 shrink-0">
                <p className="text-sm font-bold text-red-800 flex items-center gap-2"><HeartPulse size={18}/> ملاحظات طبية: {selectedEmp.chronic_disease_notes}</p>
              </div>
            )}
                  ...
                  <div key={visit.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <span className={`px-3 py-1 rounded-lg text-xs font-bold ${
                        visit.visit_type === 'Work Injury' ? 'bg-orange-100 text-orange-700' : 
                        visit.visit_type === 'First Aid' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                      }`}>{visit.visit_type}</span>
                      <span className="text-xs font-bold text-slate-400 flex items-center gap-1" dir="ltr">
                        <Clock size={12}/> {new Date(visit.created_at).toLocaleDateString('ar-EG')} - {new Date(visit.created_at).toLocaleTimeString('ar-EG', {hour:'2-digit', minute:'2-digit'})}
                      </span>
                    </div>
                    
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-sm font-semibold text-slate-700 mb-3">
                      <span className="text-slate-400 block text-xs mb-1">التشخيص / الشكوى:</span>
                      {visit.diagnosis || visit.complaint || "غير مسجل"}
                    </div>

                    {visit.visit_vitals && visit.visit_vitals.length > 0 && (
                      <div className="flex flex-wrap gap-2 text-xs font-bold">
                        {visit.visit_vitals[0].blood_pressure && <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded-md">BP: {visit.visit_vitals[0].blood_pressure}</span>}
                        {visit.visit_vitals[0].temperature && <span className="bg-orange-50 text-orange-700 px-2 py-1 rounded-md">Temp: {visit.visit_vitals[0].temperature}°C</span>}
                        {visit.visit_vitals[0].rbs && <span className="bg-emerald-50 text-emerald-700 px-2 py-1 rounded-md">RBS: {visit.visit_vitals[0].rbs}</span>}
                      </div>
                    )}
                  </div>
                ))
